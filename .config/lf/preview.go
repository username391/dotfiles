...
func main() {
	...
	os.Setenv("LESSKEYIN", filepath.Join(try1(os.UserHomeDir()), ".config/lesskey.lf"))
	...
	if opts.pagerRequired {
		...
		loop:
			for {
				...
				case 'h', 'q', 'i', 3, 10, 13, 27: // 3 is Ctrl-C, 27 is ESC, 10 is Enter, 13 is CR
					break loop
				}
			}
...
func clearKitty() {
	if os.Getenv("LF_NVIM_TERMINAL") == "1" {
		// In Neovim terminal, redirect to stderr instead of /dev/tty
		// run(`kitty +kitten icat --clear --stdin no --transfer-mode memory </dev/null >&2`)
	} else {
...
func generatePreview(cache string, opts previewOpts) string {
	...
	case opts.mime == "application/msword",
		...
		err := linuxOfficeVideoPreview(opts, cache)
		if err != nil {
...
func launchKitty(opts previewOpts, imagePath string) {
	...
	if os.Getenv("LF_NVIM_TERMINAL") == "1" {
		// In Neovim terminal, kitty +kitten icat fails to open /dev/tty
		// Use raw kitty graphics protocol instead
		displayKittyRaw(opts, imagePath)
	} else {
...
func previewArchive(opts previewOpts) {
	...
	// args := []string{"--paging=never", "--file-name=" + opts.fp}
	args := []string{}
	pg := "less"
	if opts.pagerRequired {
		pg = "bat"
		args = []string{"--paging=always", "--file-name=" + opts.fp}
	}
	cmd := exec.Command(pg, args...)
	cmd.Stdout = os.Stdout
	r, w := io.Pipe()
	cmd.Stdin = r
	go func() {
		defer w.Close()
		fsys := try1(archives.FileSystem(ctx, opts.fp, nil))
...
func linuxOfficeVideoPreview(opts previewOpts, cache string) error {
    switch opts.ext {
    ...
    case ".doc", ".docx", ".xls", ".xlsx", ".ppt", ".pptx":
        tmpDir, err := os.MkdirTemp("", "lf-office")
        if err != nil {
            return err
        }
        ...
        pdf := filepath.Join(
            tmpDir,
            strings.TrimSuffix(filepath.Base(opts.fp), opts.ext)+".pdf",
        )
